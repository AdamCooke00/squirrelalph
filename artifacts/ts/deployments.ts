...
import { SQRLFaucet, SQRLFaucetInstance } from '.'
import { default as mainnetDeployments } from '../.deployments.mainnet.json'
import { default as devnetDeployments } from '../.deployments.devnet.json'

export type Deployments = {
  deployerAddress: string
  contracts: { SQRLFaucet: DeployContractExecutionResult<SQRLFaucetInstance> }
}

function toDeployments(json: any): Deployments {
  const contracts = {
    SQRLFaucet: {
      ...json.contracts['SQRLFaucet'],
      contractInstance: SQRLFaucet.at(json.contracts['SQRLFaucet'].contractInstance.address)
    }
  }
  return {
    ...json,
...
export function loadDeployments(networkId: NetworkId, deployerAddress?: string): Deployments {
  const deployments =
    networkId === 'mainnet' ? mainnetDeployments : networkId === 'devnet' ? devnetDeployments : undefined
